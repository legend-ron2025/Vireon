...
(function overrideCursor(){
  document.addEventListener('DOMContentLoaded', function(){
    ...
    c.style.cssText = [
      ...
      'line-height:1',
      'color:#D4AF37',
      'transition:transform .15s ease, color .3s, opacity .3s',
      'animation:cursorGlow 2.5s ease-in-out infinite',
      'will-change:left,top',
      'user-select:none'
    ].join(';');

    /* Move with mouse — direct, no lag */
    document.addEventListener('mousemove', function(e){
      c.style.left = e.clientX + 'px';
      c.style.top  = e.clientY + 'px';
    }, {passive:true});

    /* Subtle scale + rotate on hover */
    document.addEventListener('mouseover', function(e){
      if(e.target.closest('a,button,.p-card,.cat-item,.btn,.vireon-logo,.ed-card')){
        c.style.transform = 'translate(-50%,-50%) scale(1.3) rotate(15deg)';
